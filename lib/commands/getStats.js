 ...
 * operators: map over all operators and add their unique stats to the list
 * general: simple hardcoded list of values
 */

// get gadget name
const opStats = OPERATORS.map(op => op.gadget);
// all other stats
const genStats = [
    "casualpvp_kills",
    "casualpvp_death",
...
const allStats = [].concat(opStats, genStats);


const baseGetter = (key, obj) => obj[`${key}:infinite`] || null;
const operatorGetter = obj => OPERATORS.reduce(function (acc, curr) {
    acc[curr.name] = {
        ...
        gadget: obj[`${curr.gadget}:${curr.fullIndex}:infinite`],
    }
    return acc;
}, {})
...
module.exports = function (ids, opts) {
    ...
    return getAuthString()
        .then(fetch(`${URLS.STATS_URL}statistics=${allStats.join(",")}&populations=${query.join(",")}`))
